...
- Do not yield to the user until the PR is merged; keep the watcher loop running
  unless blocked.
- Delete remote branches after merge when the repository does not auto-delete
  head branches.
...
5. If conflicts exist, fetch and merge the PR base branch, resolve conflicts,
   rerun validation, then use the `push` skill to publish the updated branch.
...
if [ "$mergeable" = "CONFLICTING" ]; then
  # Fetch and merge the PR base branch, resolve conflicts, rerun validation,
...
# Squash-merge, using the current PR title and body
gh pr merge --squash --subject "$pr_title" --body "$pr_body"
```

## Async Watch Helper

Preferred: use the bundled asyncio watcher from this skill directory to monitor
review comments, CI, and head updates in parallel:
...
- If CI pushes an auto-fix commit (authored by GitHub Actions), it does not
  ...
  the PR base branch if needed, add a real author commit, and force-push to retrigger
...
- Do not enable auto-merge unless the repository has required checks and the
  user explicitly wants auto-merge behavior.
