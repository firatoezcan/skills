...
> **CRITICAL**: Prefer non-exported route components. Exported component functions are included in the main bundle and bypass code splitting entirely. If a route-local component must be exported for a real external caller, it must be an Uppercase React component; never export lowercase helpers, constants, schemas, query builders, or metadata from route files.
...
// NOT exported — this preserves automatic code splitting
